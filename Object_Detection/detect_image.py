...
MODELS = {
	"frcnn-resnet": detection.fasterrcnn_resnet50_fpn(path=os.path.join(MODELS_PATH,"fasterrcnn_resnet50_fpn_coco-258fb6c6.pth"), pretrained=True, progress=True, num_classes=len(CLASSES), pretrained_backbone=True),
	"frcnn-mobilenet": detection.fasterrcnn_mobilenet_v3_large_320_fpn(path=os.path.join(MODELS_PATH,"fasterrcnn_mobilenet_v3_large_320_fpn-907ea3f9.pth"), pretrained=True, progress=True, num_classes=len(CLASSES), pretrained_backbone=True),
	"retinanet": detection.retinanet_resnet50_fpn(path=os.path.join(MODELS_PATH,"retinanet_resnet50_fpn_coco-eeacb38b.pth"), pretrained=True, progress=True, num_classes=len(CLASSES), pretrained_backbone=True)
}
